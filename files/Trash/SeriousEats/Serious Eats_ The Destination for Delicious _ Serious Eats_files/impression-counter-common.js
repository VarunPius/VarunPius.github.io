{
    ...
    for (var i=0; i < allLinks.length; i++)
    {
        ...
        else if (results = href.match(asin))
        {
            ...
            href = href.replace(/(exec\/obidos|o)\/ASIN\d?\/(\w{10})(\/ref=nosim)?\/([^\/\?]+).*/i, 'dp/$2$3?tag=$4&link_code=' + code + '&creativeASIN=$2');
            allLinks[i].href = amazon_InsertCCMID(href, code);
            var tag = results[4];
            imp = amazon_logImpression(imp, locale, tag, code);
        }
        ...
        else if (results = href.match(isbn))
        {
            ...
            href = href.replace(/(exec\/obidos|o)\/ISBN=(\w{10})(\/ref=nosim)?\/([^\/\?]+)A.*/, 'dp/$2$3?tag=$4&link_code=' + code + '&creativeASIN=$2');
            allLinks[i].href = amazon_InsertCCMID(href, code);
            var tag = results[4];
            imp = amazon_logImpression(imp, locale, tag, code);
        }
                    ...
                    && (tag_match = href.match(/[?&]t=([^&]+)/)))
        {
            ...
            href = href.replace(mpTarget, '/gp/offer-listing/$1?tag=$3&link_code=' 
                    + code + '&creativeASIN=$1');
            var mpTarget = new RegExp('/gp/associates/link-types/marketplace\\.html\\?t=([^&]+)&(amp;)?asin=(\\w{10})');
            href = href.replace(mpTarget, '/gp/offer-listing/$3?tag=$1&link_code=' 
                    + code + '&creativeASIN=$3');
            allLinks[i].href = amazon_InsertCCMID(href, code);
            imp = amazon_logImpression(imp, locale, tag, code);
        }
